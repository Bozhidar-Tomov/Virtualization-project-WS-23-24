...
def perform_lda(data, target, n_components=2):
    ...
    lda_df = pd.DataFrame(
        data=lda_components, columns=[f"LDA{i+1}" for i in range(n_components)]
    )
...
def perform_pca(data, n_components=2):
    ...
    pca_df = pd.DataFrame(
        data=components, columns=[f"PCA{i+1}" for i in range(n_components)]
    )
...
def kmeans_clustering(data, n_clusters=3):
    ...
    kmeans_df = pd.DataFrame(
        data=data, columns=[f"Feature{i+1}" for i in range(data.shape[1])]
    )
